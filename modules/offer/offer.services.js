...
// job started 
const startedJobService = async (offerId,serviceProviderId) => {
 ...
 // Check if client owns the task
  if (String(job.assignedTo) !==  serviceProviderId) {
    throw new AppError(401, "You are Not authorized to start this job");
  }

  // Ensure the task is still open
  if (job.status !== "Assigned") {
    throw new AppError(400, `Cannot start job. Job is '${job.status}'.`);
  }
